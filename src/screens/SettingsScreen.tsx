...
export default function SettingsScreen({ navigation }: Props) {
  ...
  const setup42DaysComplete = async (fmt: (d: Date) => string) => {
    ...
    challengeStart.setDate(challengeStart.getDate() - 41);
    await saveStartDate(fmt(challengeStart));
    ...
    for (let day = 1; day <= 42; day++) {
      ...
      const dur =
        20 + Math.floor(Math.sin(day * 0.4) * 15 + Math.random() * 25);
      await saveWorkout({
        id: `bg42-${day}`,
        date: fmt(date),
        dayNumber: day,
        type,
        duration: dur,
        feeling: Math.min(5, Math.max(1, 2 + Math.floor(day / 10))),
        notes: "",
      });
    }
  };
    ...
    if (trigger === "demo.2weeks") {
      ...
      for (let day = 1; day <= DAYS; day++) {
        ...
        await saveWorkout({
          ...
          feeling: Math.min(5, 3 + Math.floor(day / 6)),
          notes: "",
        });
      }
      setName("");
      ...
      Alert.alert(
        "Demo: 2 Weeks",
        "Loaded 2 weeks of progress. Check the Home screen.",
      );
      ...
      await AsyncStorage.removeItem(NAME_KEY);
      Alert.alert(
        "Demo: 42 Days Complete",
        "Challenge complete! Check the Home screen.",
      );
      ...
      const start = new Date();
      start.setDate(start.getDate() - path.weeks * 7 - 1);
      ...
      if (raw) {
        ...
        if (idx >= 0) {
          ...
          paths[idx].sessions = Array.from(
            { length: totalSessions },
            (_, i) => {
              const d = new Date(start);
              d.setDate(
                start.getDate() +
                  Math.floor((i * path.weeks * 7) / totalSessions),
              );
              return {
                date: fmt(d),
                duration: 25 + Math.floor(Math.random() * 20),
                distanceMi: parseFloat((1.5 + i * 0.1).toFixed(2)),
              };
            },
          );
      ...
      Alert.alert(
        "Demo: 5K Complete",
        "10K is now unlocked. Open the path from home.",
      );
          ...
          paths[idx].sessions = Array.from(
            ...
            (_, i) => {
              ...
              return {
                ...
                distanceMi: parseFloat((5 + i * 1.5).toFixed(2)),
              };
            },
          );
          await AsyncStorage.setItem("@42_active_path", JSON.stringify(paths));
      ...
      Alert.alert(
        "Demo: 25mi Complete",
        "50mi ride is now unlocked. Open the path from home.",
      );
          ...
          paths[idx].sessions = Array.from(
            ...
            (_, i) => {
              ...
              return {
                date: fmt(d),
                duration: 25 + Math.floor(Math.random() * 20),
                distanceMi: parseFloat((0.1 + i * 0.015).toFixed(3)),
              };
            },
          );
          await AsyncStorage.setItem("@42_active_path", JSON.stringify(paths));
      ...
      await AsyncStorage.removeItem(NAME_KEY);
      Alert.alert(
        "Demo: 500m Swim Complete",
        "1km swim is now unlocked. Open the path from home.",
      );
      ...
      if (!isNaN(n) && n >= 1 && n <= 42) {
        ...
        Alert.alert(
          `Demo: Day ${n}`,
          `Simulating ${fmt(simDate)} — Day ${n}. Existing workouts preserved.`,
        );
      }
    ...
    Alert.alert(
      ...
      [
        { text: "Cancel", style: "cancel" },
        {
          ...
          onPress: async () => {
            const path = ALL_PATHS.find((p) => p.id === "run_5k")!;
            const totalSessions = path.weeklyPlan.reduce(
              (s, w) => s + w.sessions,
              0,
            );
            ...
            if (raw) {
              ...
              if (idx >= 0) {
                ...
                for (let i = 0; i < totalSessions; i++) {
                  ...
                  d.setDate(
                    start.getDate() +
                      Math.floor((i * weeksNeeded * 7) / totalSessions),
                  );
                  sessions.push({
                    date: fmt(d),
                    duration: 25 + Math.floor(Math.random() * 20),
            ...
            Alert.alert(
              "Done!",
              "Run a 5K path loaded as complete. Open it from the home page banner. 10K is now unlocked!",
            );
          },
        },
      ],
  ...
  return (
    <View style={styles.container}>
      ...
      >
            ...
            {[6, 7, 12, 17, 18, 20].map((h) => (
              ...
              >
                <Text
                  style={[
                    styles.pillText,
                    reminderHour === h && styles.pillTextActive,
                  ]}
                >
                  {hourLabel(h)}
                </Text>
        ...
        {/* Danger zone */}
        <Text style={[styles.sectionHeader, { color: "#EF4444" }]}>
          Danger Zone
        </Text>
        <TouchableOpacity
        ...
        >
          ...
          <Text style={styles.resetSubtitle}>
            Clears all workouts, progress, milestones and restarts from Day 1.
          </Text>
        </TouchableOpacity>
